...
void print_correlations(argument_struct args, vect *fluctuations, float **dist_matrix, Boid **boid_array) {
    ...
    // std::cout << "MFV: " << velocity_to_str(mean_flock_velocity) << std::endl;
    ...
    while (true) {
        ...
        if (args.quiet) {
            ...
            if (iterations_completed <= iterations_cutoff) {
                ...
                std::cout << "num_boids: " << args.num_boids << " max_x: " << args.max_x << " max_y: " << args.max_y << " iterations: " << iterations_cutoff << std::endl; break;
            }
        }
